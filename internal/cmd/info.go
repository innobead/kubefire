...
func cniVersion(expectedVersion string) PrerequisitesInfo {
	...
	if err != nil {
		logrus.Fatalf("failed to initialize cni library: %v", err)
	}

	if err := client.Load(gocni.WithLoNetwork, gocni.WithDefaultConf); err != nil {
		logrus.Fatalf("failed to load cni configuration: %v", err)
	}

	if len(client.GetConfig().Networks) != 2 {
		logrus.Fatalln("failed to load cni configuration because network configuration count is not 2")
	}
